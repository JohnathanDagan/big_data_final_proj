...
def analyze_rain_severity_proportion(traffic_events, weather_events, city):
    print("\n" + "="*80)
    print("PROPORTION OF RAIN SEVERITY FOR EACH TRAFFIC TYPE")
    print("="*80)

    try:
        # Assuming get_city_weather_analysis exists and returns a joined DataFrame
        city_joined = get_city_weather_analysis(city, traffic_events, weather_events)
        ...
        for traffic_type in traffic_types:
            ...
            if total_count == 0:
                print(f"{traffic_type:<20} {'No data':<15} {'No data':<15} {'No data':<15} {'No data':<15}")
                continue
            ...
            for severity_num, _ in rain_severities[1:]:
                ...
                results.append(f"{proportion:.2f}%")
        ...
        for traffic_type in traffic_types:
            ...
            print(f"{traffic_type:<20} {rain_results[0]:<15} {rain_results[1]:<15} {rain_results[2]:<15}")
...
def main():
    ...
    try:
        ...
        # analyze_rain_severity_impact(traffic_events, weather_events, "Chicago")

        # analyze_rain_severity_proportion(traffic_events, weather_events, "Chicago")
        # analyze_rain_severity_proportion(traffic_events, weather_events, "Houston")
        # analyze_rain_severity_proportion(traffic_events, weather_events, "Los Angeles")
        # analyze_rain_severity_proportion(traffic_events, weather_events, "San Francisco")
        # nyc miami atlanta dallas seattle
        analyze_rain_severity_proportion(traffic_events, weather_events, "Miami")
        analyze_rain_severity_proportion(traffic_events, weather_events, "New York")
        analyze_rain_severity_proportion(traffic_events, weather_events, "Atlanta")
        analyze_rain_severity_proportion(traffic_events, weather_events, "Dallas")
        analyze_rain_severity_proportion(traffic_events, weather_events, "Seattle")
